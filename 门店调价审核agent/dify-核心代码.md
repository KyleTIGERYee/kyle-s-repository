...
> 本文件包含 Workflow 中各 Code 节点的完整 Python 代码。  
> 直接复制到 Dify Code 节点中使用。

---

## 代码段1：解析Excel文本 + 生成MCP查询SQL

**对应节点**：Code Node「解析文件 + 生成SQL」

```python
import json
import re

def main(extracted_text: str) -> dict:
    """
    解析文档提取器提取的Excel文本，生成MCP查询SQL
    
    输入：extracted_text - Doc Extractor 输出的文本
    输出：building_sql, village_sql, parsed_data
    """
    
    # ===== 配置项：MCP表名 =====
    MAIN_TABLE = "de_定价agent数据集"       # 房源主表
    DECO_TABLE = "精装简装楼栋分类"         # 装修分类表
    COST_TABLE = "应收明细表"               # 成本表
    ...
    # 遍历每一行，提取 key-value 格式的基本信息
    data_section = False  # 标记是否进入调价数据区域
    ...
    for line in lines:
        ...
        # 预处理：去掉行首尾的管道符并去除多余空格（针对Markdown表格格式）
        ...
        if data_section:
            if len(parts) >= 2:
                ...
                # 排除干扰关键词和分割线
                if nums and room_no and not any(c in room_no for c in ['房间', '调整', '价格', '---']):
                    rooms.append({
                        "room_no": room_no,
                        "adjusted_price": float(nums[0])
                    })
    ...
    # ===== 生成SQL =====
    building_sql = (
        f'SELECT "房间号", "户型", "资产户型", "定价", "是否已租", '
        f'"最后出租时间", "成交时间", "采光系数", "阳台", '
        f'"面积", "是否电梯房", "楼层", "楼栋" '
        f'FROM "{MAIN_TABLE}" '
        f"WHERE \"楼栋\" = '{building}'"
    )
    ...
    # 楼栋成本SQL：查询该楼栋的应付金额（按楼栋SUM）
    cost_sql = (
        f'SELECT "楼栋", SUM("应付金额") as "应付金额" '
        f'FROM "{COST_TABLE}" '
        f"WHERE \"楼栋\" = '{building}' "
        f'GROUP BY "楼栋"'
    )
    ...
    return {
        "building_sql": building_sql,
        "village_sql": village_sql,
        "decoration_sql": decoration_sql,
        "cost_sql": cost_sql,
...
def 执行决策树(房间数据, 楼栋房源, 全村房源, 调整后价格, 当前价格, 调价映射):
    ...
    return decision(False, 'UNKNOWN', None, '未匹配到任何规则')
...
## 代码段3：Dify Code 节点配置要点

### 变量映射注意事项

在 Dify 的 Code 节点中配置输入变量时：

1. **字符串类型变量**：Tool 节点的输出默认是字符串，直接用 `str` 类型接收
2. **变量引用语法**：`{{#节点ID.输出变量名#}}`
3. **输出必须是 dict 类型**：Code 节点的 `main` 函数必须 `return dict`

### 在 Dify 中配置 Code 节点的步骤

1. 添加 Code 节点 → 选择 Python3
2. 在「输入变量」区域添加变量，绑定上游节点输出
3. 在「输出变量」区域定义输出变量名和类型
4. 将上面的代码粘贴到代码编辑器中
5. 点击「运行」测试

### MCP 表名配置

代码段1中的 `mcp_table_name` 参数，需要：

1. 先在 Dify 中调用 `list_local_tables` 工具
2. 找到房源数据表的 `table_name`（如 `de_房源信息`）
3. 在 Code 节点的输入变量中，将 `mcp_table_name` 设为固定值

---

## 代码段4：简化版邮件微服务（方案B备选）

如果 Dify 插件市场没有合适的邮件插件，可以部署此微服务：

```python
"""
邮件发送微服务 - 部署在内网服务器
运行：pip install flask && python email_service.py
"""
...
# ===== 配置项 =====
SMTP_HOST = "smtp.your-company.com"  # 替换为公司SMTP
SMTP_PORT = 465
SMTP_USER = "audit@your-company.com"  # 替换为发件邮箱
SMTP_PASS = "your-password"           # 替换为密码/授权码
...
def send_email():
    ...
    subject = data.get('subject', '调价审核通知')
    body = data.get('body', '')
...
然后在 Dify 中添加「自定义工具」，填入 OpenAPI Schema：
