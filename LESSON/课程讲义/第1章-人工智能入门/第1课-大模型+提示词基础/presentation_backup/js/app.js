...
function initSlides() {
    ...
    for (let i = 0; i < totalSlides; i++) {
        ...
        dot.className = `slide-dot w-3 h-3 rounded-full bg-white/30 cursor-pointer transition-all ${i === 0 ? 'bg-accent scale-125' : ''}`;
...
function predictNext() {
        ...
        html += `
            <div>
                <div class="flex justify-between text-body mb-2">
                    <span>"${pred.word}"</span>
                    <span class="${i === 0 ? 'text-accent font-bold' : 'text-muted'}">${pred.prob}%</span>
                </div>
    ...
    html += `</div><div class="mt-6 p-6 bg-accent/20 rounded-xl border-2 border-accent text-center">
        <p class="text-subtitle text-accent font-bold">AI选择："${preds[0].word}"（概率最高）</p>
    </div>`;
...
function showAttention() {
    ...
    if (targetWord) {
        targetWord.style.background = 'rgba(212, 175, 55, 0.5)';
    }
